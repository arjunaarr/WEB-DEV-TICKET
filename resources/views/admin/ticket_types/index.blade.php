                ...
                <tbody>
                    @forelse ($ticketTypes as $index => $type)
                        <tr>
                            ...
                            <td>{{ $type->nama }}</td>
                            <td>
                                <button class="btn btn-sm btn-primary mr-2" onclick="openEditModal(this)" data-id="{{ $type->id }}" data-nama="{{ $type->nama }}">Edit</button>
                                <button class="btn btn-sm bg-red-500 text-white" onclick="openDeleteModal(this)" data-id="{{ $type->id }}">Hapus</button>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="3" class="text-center">Belum ada data tipe tiket.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
     ...
     <dialog id="edit_modal" class="modal">
        <form method="POST" class="modal-box">
            ...
            <div class="form-control w-full mb-4">
                ...
                <input type="text" placeholder="Masukkan nama tipe tiket" class="input input-bordered w-full" id="edit_nama" name="nama" required />
            </div>
            <div class="modal-action">
                <button class="btn btn-primary" type="submit">Simpan</button>
